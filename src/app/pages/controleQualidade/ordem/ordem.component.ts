...
gerarNumero(materialNome: string, sequencial: number): string {
 ...
 console.log('Aqui', this.analises);
  ...
  imprimirLaudoPDF(amostra_detalhes_selecionada: any){
    if(this.normalize(amostra_detalhes_selecionada.material) === 'argamassa'){
      if(this.normalize(amostra_detalhes_selecionada.tipo_amostra) === 'colante'){
...
async imprimirLaudoArgPDF(amostra_detalhes_selecionada: any) {
  ...
  if (amostra_detalhes_selecionada.expressa_detalhes) {
      ...
      this.ensaios_selecionados.forEach((selected: any) => {
        if (selected.id === ensaio_detalhes.id) {
          ...
          if (ensaio_detalhes.norma) {
            norma = ensaio_detalhes.norma;
          }
          let unidade: string = 'N/D';
          if (ensaio_detalhes.unidade) {
            unidade = ensaio_detalhes.unidade;
          }
          const linha: any[] = [];
          linha.push({ content: ensaio_detalhes.descricao });
    ...
    // Quando vem de ordem/plano
    amostra_detalhes_selecionada.ordem_detalhes.plano_detalhes.forEach((plano_detalhes: any, idxPlano: number) => {
      const body: any[] = [];
      plano_detalhes.ensaio_detalhes.forEach((ensaio_detalhes: any) => {
        this.ensaios_selecionados.forEach((selected: any) => {
          if (selected.id === ensaio_detalhes.id) {
      ...
      autoTable(doc, {
        startY: (doc as any).lastAutoTable ? (doc as any).lastAutoTable.finalY + (idxPlano === 0 ? 4 : 6) : contadorLinhas,
        head,
        body,
        theme: "grid",
        styles: { fontSize: 8, halign: 'left', cellPadding: 1 },
  ...
  const tableY =  contadorLinhas; // base na última mini-tabela
  const tableX = 14;
