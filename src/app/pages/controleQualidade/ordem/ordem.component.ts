...
gerarNumero(materialNome: string, sequencial: number): string {
  ...
  abrirModalImpressao(analise: any) {
    ...
    if(analise.amostra_detalhes.expressa_detalhes){
      ...
      const itens_argamassa = itens.map(([id, descricao]) => ({
  ...
  async imprimirLaudoArgPDF(amostra_detalhes_selecionada: any) {
    ...
    const linhas = [
      ['Material: '+amostra_detalhes_selecionada.amostra_detalhes.material, "Tipo: "+amostra_detalhes_selecionada.amostra_detalhes.tipo_amostragem, ""],
      ['Sub-Tipo: '+amostra_detalhes_selecionada.amostra_detalhes.subtipo, "Fornecedor: "+amostra_detalhes_selecionada.amostra_detalhes.fornecedor, ""],
      ['Local de Coleta: '+amostra_detalhes_selecionada.amostra_detalhes.local_coleta, "", "Data Coleta / Fabricação: "+dataColetaFormatada2],
      ['Registro EP: '+amostra_detalhes_selecionada.amostra_detalhes.registro_ep, "Registro do Produto: "+amostra_detalhes_selecionada.amostra_detalhes.registro_produto, ""],
      ['Identif. Complemento: '+amostra_detalhes_selecionada.amostra_detalhes.identificacao_complementar, "", ""],
    ];
    doc.rect(15, 48, 182, 35); // moldura
  ...
  imprimirCalculoPDF(analise: any) {
    ...
    const variaveis_argamassa: any[] = [];

    this.selectedEnsaios.forEach((selectedEnsaios: any) => {
      const id = String(selectedEnsaios.data.id);

      if (id.includes("-")) {
        const split_argamassa = id.split("-");
        variaveis_argamassa.push(split_argamassa[1]);
      }
    ...
    if (variaveis_argamassa.includes("peneira_seca")) {
      if(analise?.peneiras?.amostra){
        ...
        const body: any[] = [
          ...
          // 🔽 10 linhas vazias usando CellDef
    ...
    if (variaveis_argamassa.includes("peneira_umida")) {
    ...
    if (variaveis_argamassa.includes("superficie")) {
    ...
    if (variaveis_argamassa.includes("linear")) {
    ...
    if (variaveis_argamassa.includes("massa")) {
    ...
    if (variaveis_argamassa.includes("elasticidade")) {
